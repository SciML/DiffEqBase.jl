...
function anyeltypedual(x)
    if propertynames(x) === ()
        ...
        mapreduce(
            y -> !isdefined(x, y) ? Any : anyeltypedual(getproperty(x, y)),
            promote_dual,
            propertynames(x),
        )
    end
...
Base.@pure anyeltypedual(::Type{T}) where {T} =
...
anyeltypedual(
    x::Union{Array{T},AbstractArray{T},Set{T}},
) where {T<:Union{Number,Symbol,String}} = anyeltypedual(T)
anyeltypedual(
    x::Union{Array{T},AbstractArray{T},Set{T}},
) where {N,T<:Union{AbstractArray{<:Number},Set{<:Number},NTuple{N,<:Number}}} =
...
function promote_tspan(
    u0::AbstractArray{<:ForwardDiff.Dual},
    p,
    tspan::Tuple{<:ForwardDiff.Dual,<:ForwardDiff.Dual},
    prob,
    kwargs,
)
    return tspan
...
totallength(x::ForwardDiff.Dual) =
...
@inline ODE_DEFAULT_NORM(u::AbstractArray{<:ForwardDiff.Dual}, t::Any) =
    sqrt(sum(sse, u) / totallength(u))
@inline ODE_DEFAULT_NORM(u::ForwardDiff.Dual, ::ForwardDiff.Dual) = sqrt(sse(u))
@inline ODE_DEFAULT_NORM(u::AbstractArray{<:ForwardDiff.Dual}, ::ForwardDiff.Dual) =
    sqrt(sum(sse, u) / totallength(u))

if !hasmethod(nextfloat, Tuple{ForwardDiff.Dual})
    # Type piracy. Should upstream
    Base.nextfloat(d::ForwardDiff.Dual{T,V,N}) where {T,V,N} =
        ForwardDiff.Dual{T}(nextfloat(d.value), d.partials)
    Base.prevfloat(d::ForwardDiff.Dual{T,V,N}) where {T,V,N} =
