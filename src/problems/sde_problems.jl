...
@add_kwonly function _SDEProblem(f::AbstractSDEFunction,g,u0,
  ...
  SDEProblem{typeof(u0),typeof(_tspan),
...
function SDEProblem{iip}(f,g,u0,tspan,p=nothing;kwargs...) where {iip}
  _SDEProblem(convert(SDEFunction{iip},f,g),g,u0,tspan,p;kwargs...)
end

function SDEProblem(f::AbstractSDEFunction,u0,tspan,p=nothing;kwargs...)
  _SDEProblem(f,f.g,u0,tspan,p;kwargs...)
end

function SDEProblem(f,g,u0,tspan,p=nothing;kwargs...)
  _SDEProblem(convert(SDEFunction,f,g),g,u0,tspan,p;kwargs...)
end
...
function SplitSDEProblem{iip}(f1,f2,g,u0,tspan,p=nothing;
  ...
  _SDEProblem{iip}(SplitFunction{iip}(f1,f2,_func_cache),g,u0,tspan,p,SplitSDEProblem{iip}();kwargs...)
end
