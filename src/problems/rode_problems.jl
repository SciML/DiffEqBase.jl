...
function RODEProblem{NoiseClass,inplace,F3}(f,u0,tspan; iip = isinplace(f,4),
                     ...
                     callback=nothing,rand_prototype = nothing,mm=I)
  RODEProblem{typeof(u0),eltype(tspan),iip,NoiseClass,inplace,typeof(f),typeof(callback),typeof(mm),F3,typeof(rand_prototype)}(f,u0,tspan,noise,callback,mm,rand_prototype)
end
