...
for T in (ODEFunction,DiscreteFunction,DAEFunction,DDEFunction,RODEFunction)
  kwargs = Expr[]
  for name in fieldnames(T)
    name === :f && continue
    
    if name === :mass_matrix
      push!(kwargs, :(mass_matrix = isdefined(f, :mass_matrix) ? f.mass_matrix : I))
    else
      push!(kwargs, :($name = isdefined(f, $(QuoteNode(name))) ? f.$name : nothing))
    end
  end

  @eval begin
    Base.convert(::Type{$T}, f) = $T(f; $(kwargs...))
    Base.convert(::Type{$T{iip}}, f) where iip = $T{iip,RECOMPILE_BY_DEFAULT}(f; $(kwargs...))
  end
end

for T in (SDEFunction,)
  kwargs = Expr[]
  for name in fieldnames(T)
    (name === :f || name === :g) && continue
  ...
  @eval begin
    Base.convert(::Type{$T}, f, g) = $T(f, g; $(kwargs...))
    Base.convert(::Type{$T{iip}}, f, g) where iip = $T{iip,RECOMPILE_BY_DEFAULT}(f, g; $(kwargs...))
  end
end