...
@inline UNITLESS_ABS2(x::AbstractArray) =
    mapreduce(UNITLESS_ABS2, abs2_and_sum, x, init = zero(real(value(eltype(x)))))
@inline UNITLESS_ABS2(x::RecursiveArrayTools.ArrayPartition) =
...
@inline function ODE_DEFAULT_NORM(u::Array{T}, t) where {T<:Union{AbstractFloat,Complex}}
    x = abs2(u[1])
    @inbounds for i = 2:length(u)
        x += abs2(u[i])
    end
    sqrt(real(x) / length(u))
end

@inline ODE_DEFAULT_NORM(
    u::StaticArrays.StaticArray{T},
    t,
) where {T<:Union{AbstractFloat,Complex}} = sqrt(real(sum(abs2, u)) / length(u))
...
@inline NAN_CHECK(x::SparseArrays.AbstractSparseMatrixCSC) =
