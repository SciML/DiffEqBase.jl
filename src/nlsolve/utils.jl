...
DiffEqBase.@def iipnlsolve begin
    ...
    elseif alg.nlsolve isa NLAnderson
        ...
        Δz₊s = [zero(z) for i = 1:max_history]
        ...
        nlcache = DiffEqBase.NLAndersonCache(
            z₊,
            dzold,
            z₊old,
            Δz₊s,
    ...
    nlsolver = NLSolver{
        ...
        typeof(jac_config),
        typeof(linsolve),
        typeof(fast_convergence_cutoff),
        typeof(nlcache),
    }(
        z,
        dz,
...
DiffEqBase.@def oopnlsolve begin
    ...
    elseif alg.nlsolve isa NLAnderson
        ...
        nlcache = DiffEqBase.NLAndersonConstantCache(
            Δz₊s,
            Q,
            R,
            γs,
...
function nlsolve_resize!(integrator::DEIntegrator, i::Int)
    ...
    if nlsolver isa AbstractArray
        for idx in eachindex(nlsolver) # looping because we may have multiple nlsolver for threaded case
            ...
            @unpack z, dz, tmp, ztmp, k, du1, uf, jac_config, linsolve, weight, cache =
                _nlsolver
...
function nlsolve_cache_resize!(cache::NLAndersonCache, alg, i::Int)
    ...
    if max_history > prev_max_history
        for i = (max_history-prev_max_history):max_history
            cache.Δz₊s[i] = zero(z₊)
        end
    end
