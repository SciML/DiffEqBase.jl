...
DiffEqBase.@def iipnlsolve begin
    ...
    if alg.nlsolve isa NLNewton
        ...
        if islinear(f)
            ...
            if DiffEqBase.has_jac(f) && !DiffEqBase.has_Wfact(f) &&
    ...
    elseif alg.nlsolve isa NLAnderson
        ...
        nlcache = DiffEqBase.NLAndersonCache(z₊, dzold, z₊old, Δz₊s, Q, R, γs,
                                             alg.nlsolve.aa_start, alg.nlsolve.droptol)
    end
    ...
    nlsolver = NLSolver{true, typeof(z), typeof(k), uTolType, typeof(κ), typeof(γ),
                        typeof(c), typeof(du1), typeof(uf), typeof(jac_config),
                        typeof(linsolve), typeof(fast_convergence_cutoff), typeof(nlcache)}(z,
                                                                                            dz,
                                                                                            tmp,
                                                                                            b,
                                                                                            k,
...
DiffEqBase.@def oopnlsolve begin
    ...
    if alg.nlsolve isa NLNewton
        ...
        if islinear(f) || DiffEqBase.has_jac(f)
            ...
            if u isa StaticArray
                ...
                W = u isa Number ? u :
                    LU{LinearAlgebra.lutype(uEltypeNoUnits)}(Matrix{uEltypeNoUnits}(undef,
                                                                                    0, 0),
    ...
    elseif alg.nlsolve isa NLAnderson
        ...
        nlcache = DiffEqBase.NLAndersonConstantCache(Δz₊s, Q, R, γs, alg.nlsolve.aa_start,
                                                     alg.nlsolve.droptol)
    end

    # create non-linear solver
    nlsolver = NLSolver{false, typeof(z), typeof(k), uTolType, typeof(κ), typeof(γ),
                        typeof(c), Nothing, typeof(uf), Nothing, Nothing,
                        typeof(fast_convergence_cutoff), typeof(nlcache)}(z, dz, tmp, b, k,
                                                                          one(uTolType), κ,
                                                                          γ, c,
                                                                          alg.nlsolve.max_iter,
                                                                          10000,
...
# No J version
function iipnlsolve(alg, u, uprev, p, t, dt, f, W, rate_prototype, uEltypeNoUnits,
                    uBottomEltypeNoUnits, γ, c)
    iipnlsolve(alg, u, uprev, p, t, dt, f, W, nothing, rate_prototype, uEltypeNoUnits,
               uBottomEltypeNoUnits, γ, c)
end

function iipnlsolve(alg, u, uprev, p, t, dt, f, W, J, rate_prototype, uEltypeNoUnits,
                    uBottomEltypeNoUnits, γ, c)
    @unpack κ, fast_convergence_cutoff = alg.nlsolve
...
# No J version
function oopnlsolve(alg, u, uprev, p, t, dt, f, W, rate_prototype, uEltypeNoUnits,
                    uBottomEltypeNoUnits, γ, c)
    oopnlsolve(alg, u, uprev, p, t, dt, f, W, nothing, rate_prototype, uEltypeNoUnits,
               uBottomEltypeNoUnits, γ, c)
end

function oopnlsolve(alg, u, uprev, p, t, dt, f, W, J, rate_prototype, uEltypeNoUnits,
                    uBottomEltypeNoUnits, γ, c)
    @unpack κ, fast_convergence_cutoff = alg.nlsolve
...
DiffEqBase.@def getoopnlsolvefields begin uf = nlsolver.uf end
