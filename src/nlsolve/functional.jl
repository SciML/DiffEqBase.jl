...
"""
@muladd function nlsolve!(nlsolver::NLSolver,
    ...
    while iter < max_iter
        ...
        if nlcache isa NLAndersonConstantCache && iter < max_iter
            ...
            elseif iter > aa_start
                ...
                atmp = calculate_residuals(z .- z₊ .+ dz, uprev, u, integrator.opts.abstol,
...
@muladd function nlsolve!(nlsolver::NLSolver,
                          nlcache::Union{NLFunctionalCache, NLAndersonCache}, integrator)
    @unpack t, dt, uprev, u, p = integrator
    ...
    while iter < max_iter
        ...
        # compute norm of residuals
        iter > 1 && (ndzprev = ndz)
        calculate_residuals!(ztmp, dz, uprev, u, integrator.opts.abstol,
                             integrator.opts.reltol, integrator.opts.internalnorm, t)
