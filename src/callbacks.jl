...
function initialize!(u, t, integrator::DEIntegrator, any_modified::Bool, c::DECallback)
    c.initialize(c, u, t, integrator)
...
function finalize!(u, t, integrator::DEIntegrator, any_modified::Bool, c::DECallback)
    c.finalize(c, u, t, integrator)
    any_modified || integrator.u_modified
end
...
function find_first_continuous_callback(
    integrator,
    callback::AbstractContinuousCallback,
    args...,
)
    find_first_continuous_callback(
        integrator,
        find_callback_time(integrator, callback, 1)...,
        1,
        1,
        args...,
    )
...
function find_first_continuous_callback(
    ...
    tmin2, upcrossing2, event_occurred2, event_idx2 =
        find_callback_time(integrator, callback2, counter)
...
@inline function determine_event_occurance(
    integrator,
    ...
    previous_condition =
        @views(integrator.callback_cache.previous_condition[1:callback.len])
    ...
    if callback.idxs === nothing
        ...
        callback.condition(
            previous_condition,
            integrator.uprev[callback.idxs],
            integrator.tprev,
            integrator,
        )
    end
    ...
    if integrator.event_last_time == counter &&
       minimum(
        ODE_DEFAULT_NORM(
            ArrayInterfaceCore.allowed_getindex(previous_condition, ivec),
            integrator.t,
        ),
    ...
    event_idx =
        findall_events!(next_sign, callback.affect!, callback.affect_neg!, prev_sign)
    if sum(event_idx) != 0
        event_occurred = true
...
@inline function determine_event_occurance(
    ...
    if callback.idxs === nothing
        previous_condition =
            callback.condition(integrator.uprev, integrator.tprev, integrator)
    else
        @views previous_condition = callback.condition(
            integrator.uprev[callback.idxs],
            integrator.tprev,
            integrator,
        )
...
function bisection(
    ...
    if rootfind == SciMLBase.LeftRootFind
        NonlinearSolve.solve(
            NonlinearSolve.NonlinearProblem{false}(f, tup),
...
function findall_events!(
    ...
    @inbounds for i = 1:length(prev_sign)
        next_sign[i] =
            (
...
function find_callback_time(integrator, callback::ContinuousCallback, counter)
    event_occurred, interp_index, ts, prev_sign, prev_sign_index, event_idx =
        determine_event_occurance(integrator, callback, counter)
    if event_occurred
                ...
                if zero_func(top_t) == 0
                    ...
                    if integrator.event_last_time == counter &&
                       ...
                       prev_sign_index == 1
                        ...
                        sign(zero_func(bottom_t)) * sign_top >= zero(sign_top) && error(
...
function find_callback_time(integrator, callback::VectorContinuousCallback, counter)
    ...
    if event_occurred
        if callback.condition === nothing
            ...
            if callback.rootfind != SciMLBase.NoRootFind && !isdiscrete(integrator.alg)
                ...
                min_event_idx = -1
                for idx = 1:length(event_idx)
                    if ArrayInterfaceCore.allowed_getindex(event_idx, idx) != 0
                        zero_func(abst, p = nothing) = ArrayInterfaceCore.allowed_getindex(
                        ...
                        if zero_func(top_t) == 0
                            ...
                            if integrator.tdir * Θ < integrator.tdir * min_t
                                integrator.last_event_error =
                                    ODE_DEFAULT_NORM(zero_func(Θ), Θ)
                            end
                        end
                        if integrator.tdir * Θ < integrator.tdir * min_t
                            min_event_idx = idx
    ...
    new_t,
    ArrayInterfaceCore.allowed_getindex(prev_sign, min_event_idx),
    event_occurred::Bool,
    min_event_idx::Int
end
...
@inline function apply_discrete_callback!(
    ...
    bool, saved_in_cb2 = apply_discrete_callback!(
        integrator,
        apply_discrete_callback!(integrator, callback)...,
        args...,
    )
