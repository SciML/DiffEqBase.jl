...
function init_call(
    ...
    if has_kwargs(_prob)
        if merge_callbacks && haskey(_prob.kwargs, :callback) && haskey(kwargs, :callback)
            kwargs_temp =
                NamedTuple{Base.diff_names(Base._nt_names(values(kwargs)), (:callback,))}(
                    values(kwargs),
                )
            callbacks = NamedTuple{(:callback,)}((
                DiffEqBase.CallbackSet(_prob.kwargs[:callback], values(kwargs).callback),
            ))
    ...
    if hasfield(typeof(_prob), :f) &&
       hasfield(typeof(_prob.f), :f) &&
...
function solve_call(
    ...
    isdefined(_prob, :u0) &&
        ...
        throw(
            NonConcreteEltypeError(RecursiveArrayTools.recursive_unitless_eltype(_prob.u0)),
        )
...
function solve(prob::PDEProblem, alg::DiffEqBase.DEAlgorithm, args...; kwargs...)
    solve(prob.prob, alg, args...; kwargs...)
end

function init(prob::PDEProblem, alg::DiffEqBase.DEAlgorithm, args...; kwargs...)
...
function get_concrete_problem(prob::DAEProblem, isadapt; kwargs...)
    ...
    if isconcreteu0(prob, tspan[1], kwargs) &&
       typeof(u0_promote) === typeof(prob.u0) &&
       isconcretedu0(prob, tspan[1], kwargs) &&
       typeof(du0_promote) === typeof(prob.du0) &&
       prob.tspan == tspan &&
       typeof(prob.tspan) === typeof(tspan_promote) &&
       p === prob.p &&
       f_promote === prob.f
...
function check_prob_alg_pairing(prob, alg)
    ...
    if isdefined(prob, :u0) &&
       eltype(prob.u0) <: ForwardDiff.Dual &&
               ...
               !(uType <: Union{Float32,Float64,ComplexF32,ComplexF64})
                ...
                throw(
                    GenericNumberTypeError(
                        alg,
                        isdefined(prob, :u0) ? prob.u0 : nothing,
...
@deprecate concrete_solve(
    ...
    args...;
    kwargs...,
) solve(prob, alg, args...; u0 = u0, p = p, kwargs...)
