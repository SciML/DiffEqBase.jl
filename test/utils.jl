...
@testset "tspan2dtmin" begin
    ...
    @test approxoftype(tspan2dtmin((10, 100.0)), 10*eps(100.0))
    @test approxoftype(tspan2dtmin((-10000.0, 100.0)), 10*eps(10000.0))
    @test tspan2dtmin((1, 2)) === 0
    @test approxoftype(tspan2dtmin((1 // 10, 2 // 10)), 1 // 2^33)
    @test approxoftype(tspan2dtmin((2 // 10, Inf)), 10*eps(.2))
    @test approxoftype(tspan2dtmin((2 // 1, Inf)), 10*eps(2.0))
    @test approxoftype(tspan2dtmin((0, Inf)), eps(1.0))
    @test approxoftype(tspan2dtmin((0.0, Inf)), eps(1.0))
    @test approxoftype(tspan2dtmin((0.0, 1e-6)), 10*eps(1e-6))
